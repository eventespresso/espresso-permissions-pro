...
if (!function_exists('add_event_espresso_menus')) {
	function add_event_espresso_menus() {
		global $espresso_manager, $org_options;

		//Main menu tab
		add_menu_page(__('Event Espresso','event_espresso'), '<span style=" font-size:12px">'.__('Event Espresso','event_espresso').'</span>', $espresso_manager['espresso_manager_general'], 'event_espresso', 'organization_config_mnu', EVENT_ESPRESSO_PLUGINFULLURL.'images/events_icon_16.png');

		//General Settings
		//if (espresso_member_data('role')==$espresso_manager['espresso_manager_general']||espresso_member_data('role')=='administrator')
		...
		if (function_exists('espresso_fb_settings')) {
		...
		//Reports
		if (function_exists('espresso_reports')) {
		...
		if (function_exists('espresso_social_config_mnu')) {
		...
		//Addons
		add_submenu_page('event_espresso', __('Event Espresso - Addons','event_espresso'), __('Addons','event_espresso'), apply_filters( 'espresso_management_capability', 'administrator', $espresso_manager['espresso_manager_addons'] ), 'admin_addons', 'event_espresso_addons_mnu');
		...
		add_submenu_page('event_espresso', __('Event Espresso - Event Manager Permissions','event_espresso'), __('Permissions','event_espresso'), 'administrator', 'espresso_permissions', 'espresso_permissions_config_mnu');
...
function espresso_manager_install(){
	...
	$espresso_manager = array(
					...
					'event_manager_create_post'=>'N',
					'event_manager_share_cats'=>'N',
				);
	add_option( 'espresso_manager_settings', $espresso_manager );
	// add more capabilities to the subscriber role only for this theme.
...
//							if ( !isset($avail_roles[$this_role]) ) continue;
		...
		?>
          <div class="postbox">
              ...
              <div class="inside">
              <p><?php _e('Events created by "Event Managers" require approval?', 'event_espresso'); ?> <?php echo select_input('event_manager_approval', $values_b, $espresso_manager['event_manager_approval']);?></p>
              <p><?php _e('Regional managers can edit venues assigned to them?', 'event_espresso'); ?> <?php echo select_input('event_manager_venue', $values_b, $espresso_manager['event_manager_venue']);?></p>
              <p><?php _e('Regional managers are in charge only of their staff?', 'event_espresso'); ?> <?php echo select_input('event_manager_staff', $values_b, $espresso_manager['event_manager_staff']);?></p>
              <p><?php _e('Anyone can create a post when publishing an event?', 'event_espresso'); ?> <?php echo select_input('event_manager_create_post', $values_b, $espresso_manager['event_manager_create_post']);?></p>
              <p><?php _e('Enable sharing of categories between users?', 'event_espresso'); ?> <?php echo select_input('event_manager_share_cats', $values_b, $espresso_manager['event_manager_share_cats']);?></p>
              </div>
         </div>
